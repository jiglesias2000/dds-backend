...
async function CrearBaseSiNoExiste() {
  try {
    ...
    [rows] = await connection.execute(
      "SELECT COUNT(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name = ?",
      ["usuarios"]
    );

    existe = rows[0].contar > 0;
    ...
    [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='articulosfamilias'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE table articulosfamilias( IdArticuloFamilia INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR(50) NOT NULL UNIQUE);"
      );
    ...
    existe = false;
    sql =
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='articulos'";
    ...
    if (!existe) {
      await connection.execute(
        `CREATE table articulos( 
              ...
              , CodigoDeBarra VARCHAR(60) NOT NULL
              , IdArticuloFamilia INT NOT NULL
              , Stock INT NOT NULL
  ...
  `);
    ...
    existe = false;
    [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='Jugadores'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE TABLE IF NOT EXISTS Jugadores (IdJugador INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR(60) NOT NULL, Apellido VARCHAR(60) NOT NULL, FechaNacimiento DATE NOT NULL, Activo BOOLEAN NOT NULL);"
      );
      await connection.execute(
        "INSERT INTO Jugadores (Nombre, Apellido, FechaNacimiento, Activo) VALUES ('Leo', 'Messi', '1990-01-19', 1), ('Lautaro', 'Martinez', '1990-01-25', 1), ('Julian', 'Alvarez', '1995-04-30', 1), ('Miguel', 'Borja', '1996-06-20', 1), ('Leo', 'Paredes', '1989-09-25', 1), ('Santiago', 'Simon', '1987-09-20', 1), ('Nicolas', 'tagliafico', '1988-12-1', 1), ('Alexis', 'McAllister', '1990-08-13', 1), ('Enzo', 'Fernandez', '1997-12-01', 1), ('Paulo', 'Dybala', '1996-10-01', 1);"
      );
    ...
    existe = false;
     [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='Ligas'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE TABLE IF NOT EXISTS Ligas (IdLiga INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR (100) NOT NULL, FechaInicio DATE NOT NULL);"
      );
      await connection.execute(
        "INSERT INTO Ligas (IdLiga, Nombre, FechaInicio) VALUES (1,'Primera Division', '2023-12-1'), (2,'B Nacional','2023-11-1'), (3,'Premier League','2023-10-16'), (4,'Serie A','2023-04-16'), (5,'La Liga','2024-01-20'), (6,'Bundesliga','2023-10-19'), (7,'Brasileirao','2023-09-15'), (8,'MLS','2023-07-15'), (9,'Serie B','2024-02-18'), (10,'League One','2023-08-13');"
      );
    ...
    existe = false;
    [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='Equipos'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE TABLE IF NOT EXISTS Equipos (IdEquipo INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR(100) NOT NULL, Precio REAL, CdadJugadores INTEGER, FechaAlta DATE, Activo BOOLEAN NOT NULL);"
      );
      await connection.execute(
    ...
    existe = false;
    [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='Copas'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE TABLE IF NOT EXISTS Copas (IdCopa INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR (100) NOT NULL, FechaInicio DATE NOT NULL);"
      );
      await connection.execute(
    ...
    existe = false;
    [rows] = await connection.execute(
      "SELECT count(*) as contar FROM information_schema.tables WHERE table_schema = 'pymes'  AND table_name ='Estadios'",
      []
    );
   existe = rows[0].contar > 0;
    if (!existe) {
      await connection.execute(
        "CREATE TABLE IF NOT EXISTS Estadios (IdEstadio INT PRIMARY KEY AUTO_INCREMENT , Nombre VARCHAR (100) NOT NULL, FechaInauguracion DATE NOT NULL);"
      );
      await connection.execute(
